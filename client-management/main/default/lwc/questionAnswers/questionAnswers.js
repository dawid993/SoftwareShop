...
export default class QuestionAnswers extends LightningElement {
    ...
    renderedCallback() {
        if (this._selectedAnswers) {
            this.template
                .querySelector(this.getCurrentAnswerComponentName())
                .selectAnswers(this._selectedAnswers);
        }
    }
    ...
    getSelectedAnswers() {
        ...
        if (!component) {
            throw new Error(
                "Something went wrong. Cannot find answer component."
            );
        }
